...
To create a UDF, use the `createFunction` method on the SQL interface:

```javascript
// In a Durable Object
storage.sql.createFunction('myFunction', (arg1, arg2) => {
...
storage.sql.createFunction('distance', (x1, y1, x2, y2) => {
...
storage.sql.createFunction('abs', (value) => {
...
storage.sql.createFunction('validateEmail', (email) => {
...
```javascript
storage.sql.deleteFunction('myFunction');
```
...
export class MyDurableObject {
  ...
  registerCustomFunctions() {
    ...
    sql.createFunction('formatCurrency', (amount, currency = 'USD') => {
    ...
    sql.createFunction('calculateTax', (amount, rate = 0.0725) => {
      return amount * rate;
    });
  }
}
...
storage.sql.createFunction('typeInfo', (value) => {
...
storage.sql.createFunction('highlight', (text) => `<mark>${text}</mark>`);
...
```javascript
// Define the function
storage.sql.createFunction('complexCalculation', (a, b, c) => {
  // Complex logic here
  return result;
});
...
```javascript
storage.sql.createFunction('debugFunc', (value) => {
