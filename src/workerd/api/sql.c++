...
void SqlStorage::createScalarFunction(
  ...
  auto callback = [funcRef = jsg::JsRef<jsg::JsValue>(js, functionValue), isolate = js.v8Isolate,
    ...
    if (maybeResult.IsEmpty()) {
      ...
      v8::Local<v8::String> messageKey = v8::String::NewFromUtf8(isolate, "message").ToLocalChecked();
      ...
      if (jsException->IsObject()) {
        ...
        if (jsExceptionObj->Has(context, messageKey).FromMaybe(false)) {
          v8::Local<v8::Value> messageValue = jsExceptionObj->Get(context, messageKey).ToLocalChecked();
        ...
        lastErrorMessage = kj::str(errorType, " in SQL UDF '", functionNameCopy, "': ", errorMessage);
      ...
      kj::throwFatalException(kj::Exception(kj::Exception::Type::FAILED, 
          "SQL UDF", __LINE__, kj::str("JavaScript error in UDF '", functionNameCopy, "'")));
    }
    ...
    if (jsResult->IsNull() || jsResult->IsUndefined()) {
      ...
      if (byteLength > 0) {
        memcpy(copy.begin(), 
               static_cast<const byte*>(backingStore->Data()) + byteOffset, 
               byteLength);
      }
...
void SqlStorage::onError(kj::Maybe<int> sqliteErrorCode, kj::StringPtr message,
  ...
  if (hasJsException) {
    ...
    v8::Local<v8::String> errorMsg = v8::String::NewFromUtf8(isolate, 
        lastErrorMessage.begin(), v8::NewStringType::kNormal, 
        lastErrorMessage.size()).ToLocalChecked();
    ...
    if (lastErrorStack.size() > 0) {
      ...
      v8::Local<v8::String> stackKey = v8::String::NewFromUtf8(isolate, "stack").ToLocalChecked();
      v8::Local<v8::String> stackValue = v8::String::NewFromUtf8(isolate, 
          lastErrorStack.begin(), v8::NewStringType::kNormal, 
          lastErrorStack.size()).ToLocalChecked();
