...
export class DurableObjectExample extends DurableObject {
  ...
  async testBasicFunctionality() {
    ...
    sql.createScalarFunction('identity', (input) => input);
    ...
    sql.createScalarFunction('reverse_string', (input) => {
      if (typeof input !== 'string') return input;
      return input.split('').reverse().join('');
    });
    this.functionsTeardown.push('reverse_string');
    
    const reverseResult = sql.exec('SELECT reverse_string(value) AS result FROM basic_test').toArray();
    results.push({
      test: 'string_operations',
      success: reverseResult.length === 1 && reverseResult[0].result === 'eulav tset'
    });
    
    // Test 3: Function with multiple arguments
    sql.createScalarFunction('add_numbers', (a, b) => {
      return Number(a) + Number(b);
    });
    this.functionsTeardown.push('add_numbers');
    
    const addResult = sql.exec('SELECT add_numbers(10, 32) AS result').toArray();
    results.push({
    ...
    sql.createScalarFunction('handle_null', (input) => {
      return input === null ? 'was null' : 'not null';
    });
    this.functionsTeardown.push('handle_null');
    
    const nullResult = sql.exec('SELECT handle_null(NULL) AS result1, handle_null(42) AS result2').toArray();
    results.push({
      test: 'null_handling',
      success: nullResult.length === 1 && 
    ...
    for (const result of results) {
      if (!result.success) {
        throw new Error(`Test '${result.test}' failed: ${JSON.stringify(result)}`);
      }
    }
  ...
  async testErrorHandling() {
    ...
    sql.createScalarFunction('will_throw', (input) => {
    ...
    sql.createScalarFunction('will_throw_custom_type', (input) => {
    ...
    sql.createScalarFunction('will_throw_property', (input) => {
    ...
    sql.createScalarFunction('outer_function', (input) => {
      function level1() {
        function level2() {
          function level3() {
    ...
    try {
      ...
      results.push({ test: 'explicit_error', success: false, message: 'Should have thrown' });
    } catch (e) {
    ...
    try {
      ...
      results.push({ test: 'custom_error_type', success: false, message: 'Should have thrown' });
    } catch (e) {
      // Log the full error details for debugging
      ...
      results.push({ 
        ...
        message: e.message,
        hasStack: !!e.stack,
        includesUDFName: e.message && e.message.includes('will_throw_custom_type'),
    ...
    try {
      ...
      results.push({ test: 'property_error', success: false, message: 'Should have thrown' });
    } catch (e) {
      // Log the full error details for debugging
    ...
    try {
      ...
      results.push({ test: 'nested_error', success: false, message: 'Should have thrown' });
    } catch (e) {
      // Log the full error details for debugging
    ...
    for (const result of results) {
      ...
      if (!result.hasStack) {
        throw new Error(`Test '${result.test}' failed: Exception is missing stack trace`);
      }
      
      if (result.test === 'explicit_error') {
        if (!result.stackIncludesUDF) {
          throw new Error(`Test '${result.test}' failed: Stack trace doesn't include UDF function name`);
        }
        if (!result.includesUDFName) {
          throw new Error(`Test '${result.test}' failed: Error message doesn't include UDF function name`);
        }
      }
      ...
      if (result.test === 'custom_error_type') {
        ...
        if (!result.includesErrorType) {
          throw new Error(`Test '${result.test}' failed: Error message doesn't include custom error type`);
        }
      }
      ...
      if (result.test === 'property_error') {
        ...
        if (!result.includesTypeError) {
          throw new Error(`Test '${result.test}' failed: Error message doesn't include TypeError type`);
        }
      }
      
      if (result.test === 'nested_error') {
        if (!result.stackIncludesLevel1 || !result.stackIncludesLevel2 || !result.stackIncludesLevel3) {
          throw new Error(`Test '${result.test}' failed: Stack trace doesn't include all nested function calls`);
        }
        if (!result.includesUDFName) {
          throw new Error(`Test '${result.test}' failed: Error message doesn't include UDF function name`);
        }
  ...
  async testHibernation() {
    ...
    sql.exec('CREATE TABLE hibernation_test (id INTEGER PRIMARY KEY, value TEXT, counter INTEGER)');
    
    // Insert some test data
    sql.exec('INSERT INTO hibernation_test (value, counter) VALUES (?, ?)', 'initial value', 0);
    
    // Register UDFs
    // Register a simple function that returns a constant
    sql.createScalarFunction('get_constant', () => {
      return 'persistent value';
    });
    this.functionsTeardown.push('get_constant');
    
    // Register a function that increments a counter
    sql.createScalarFunction('increment_counter', (value) => {
      return (value || 0) + 1;
    });
    this.functionsTeardown.push('increment_counter');
    
    // Register a more complex function that transforms data
    sql.createScalarFunction('transform_data', (input) => {
    ...
    const before = {
      ...
      transformed: [...sql.exec("SELECT transform_data('test-data') as value")][0].value,
  ...
  async testLimitsContexts() {
    ...
    sql.exec('CREATE TABLE test_limits (id INTEGER PRIMARY KEY, value INTEGER)');
  ...
  async testFunctionCountLimits() {
    ...
    for (let i = 0; i < MAX_ALLOWED; i++) {
      const funcName = `test_func_${i}`;
      sql.createScalarFunction(funcName, (x) => x);
      this.functionsTeardown.push(funcName);
    }
    console.log(`Registered ${MAX_ALLOWED} functions in ${Date.now() - start}ms`);
    
    // Register one more function - should work
    sql.createScalarFunction('one_more_function', (x) => x);
    this.functionsTeardown.push('one_more_function');
    console.log(`Successfully registered one more function (total: ${MAX_ALLOWED + 1})`);
    
    // Try to register one more function - should fail with limit error
    try {
      sql.createScalarFunction('exceeds_limit', (x) => x);
      ...
      if (e.message && e.message.includes("Maximum number of user-defined functions")) {
  ...
  async testCpuLimits() {
    ...
    sql.createScalarFunction('cpu_intensive', (n) => {
    ...
    try {
      ...
      const mediumCpuResult = [...sql.exec('SELECT cpu_intensive(1000) as result')];
      console.log(`Medium CPU test result: ${typeof mediumCpuResult[0].result}`);
    } catch (e) {
  ...
  async testMemoryLimits() {
    ...
    sql.createScalarFunction('memory_intensive', (sizeMB) => {
      ...
      try {
        ...
        const duration = Date.now() - start;
        console.log(`Memory allocation of ${sizeMB}MB completed in ${duration}ms`);
    ...
    try {
      ...
      const smallMemResult = [...sql.exec('SELECT memory_intensive(1) as result')];
      console.log(`Small memory test result: ${smallMemResult[0].result} bytes`);
      
      console.log("Testing with medium memory allocation (10MB, should succeed)");
      const mediumMemResult = [...sql.exec('SELECT memory_intensive(10) as result')];
      console.log(`Medium memory test result: ${mediumMemResult[0].result} bytes`);
    } catch (e) {
  ...
  async testLimitsInTransaction() {
    ...
    console.log("Transaction tests would verify that UDF limits are enforced in transactions");
  }
  ...
  async testLimitsInPreparedStatements() {
    ...
    sql.createScalarFunction('cpu_intensive_prep', (n) => {
      let result = 0;
      for (let i = 0; i < n; i++) {
        for (let j = 0; j < 1000; j++) {
          result += Math.sqrt(i * j) / (i + 1);
    ...
    try {
      ...
      console.log(`Prepared statement with low CPU usage succeeded: ${typeof lowCpuResult[0].result}`);
    } catch (e) {
  ...
  async runBaselineTests() {
    ...
    console.log(`- SELECT with filtering: ${duration / iterations}ms per iteration`);
    ...
    duration = Date.now() - start;
    console.log(`- SELECT with calculation: ${duration / iterations}ms per iteration`);
  }
  ...
  registerPerformanceUdfs() {
    ...
    // Double function
    sql.createScalarFunction('double', (x) => x * 2);
    this.functionsTeardown.push('double');
    
    // Square function
    sql.createScalarFunction('square', (x) => x * x);
    this.functionsTeardown.push('square');
    
    // Simple text function
    sql.createScalarFunction('prefix', (x) => `value_${x}`);
    this.functionsTeardown.push('prefix');
    
    // Complex computation
    sql.createScalarFunction('factorial', (n) => {
      if (n <= 1) return 1;
      let result = 1;
    ...
    sql.createScalarFunction('makeBlob', (n) => {
  ...
  async runUdfTests() {
    ...
    let start = Date.now();
    for (let i = 0; i < iterations; i++) {
      [...sql.exec('SELECT id, value, double(value) AS doubled FROM test_data')];
    }
    let duration = Date.now() - start;
    ...
    start = Date.now();
    for (let i = 0; i < iterations; i++) {
      [...sql.exec('SELECT id, value, factorial(value % 10) FROM test_data LIMIT 50')];
    }
    duration = Date.now() - start;
  ...
  async testPromiseBehavior() {
    ...
    try {
      ...
      if (!(error instanceof TypeError) || !error.message.includes('Async functions are not supported')) {
        throw new Error(`Expected TypeError about async functions, got: ${error.constructor.name}: ${error.message}`);
      }
    }
    ...
    try {
      ...
      try {
        ...
        console.log('Got expected execution error for Promise function:', e.message);
        if (!e.message.includes('Functions that return Promises are not supported')) {
          throw new Error(`Expected error about Promise returns during execution, got: ${e.message}`);
        }
      }
    } catch (e) {
      ...
      throw new Error(`Unexpected error registering Promise function: ${e.message}`);
    }
    ...
    try {
      sql.createScalarFunction('conditionalPromiseFunc', (shouldReturn) => {
        if (shouldReturn) {
      ...
      try {
        const result = [...sql.exec('SELECT conditionalPromiseFunc(1) as result')];
        throw new Error('Should not reach here - conditionalPromiseFunc(1) should throw');
      } catch (e) {
        console.log('Got expected execution error for conditional Promise function:', e.message);
        if (!e.message.includes('Functions that return Promises are not supported')) {
          throw new Error(`Expected error about Promise returns for conditional, got: ${e.message}`);
        }
      }
      
      // Trying with shouldReturn=false should work
      const result = [...sql.exec('SELECT conditionalPromiseFunc(0) as result')];
      if (result[0].result !== 42) {
        throw new Error(`Expected result 42 from conditionalPromiseFunc(0), got ${result[0].result}`);
      }
    } catch (e) {
      if (e.message.includes('Should not reach here')) {
        throw e;
      }
      throw new Error(`Unexpected error with conditional Promise function: ${e.message}`);
    }
    
    // Test function that creates but doesn't return a Promise - should work
    let promiseCreatorRegistered = false;
    try {
      sql.createScalarFunction('promiseCreator', () => {
      ...
      throw new Error(`Function creating but not returning Promise should not be rejected: ${e.message}`);
    }
    
    // Test regular function with no Promise - should work fine
    let regularFuncRegistered = false;
    try {
      sql.createScalarFunction('regularFunc', () => 42);
  ...
  async testCpuLimit() {
    ...
    sql.createScalarFunction('cpuIntensive', () => {
      ...
      try {
        ...
        console.log(`CPU intensive operation completed in ${result.duration}ms`);
    ...
    try {
      ...
      if (e.message && (e.message.includes('exceeded CPU time') || 
  ...
  async testMemoryLimit() {
    ...
    sql.createScalarFunction('memoryIntensive', () => {
  ...
  async testNumericEdgeCases() {
    ...
    sql.createScalarFunction('returnLargeInt', () => {
    ...
    console.log(`Large integer result: ${result[0].result}, type: ${typeof result[0].result}`);
    
    // Test special numeric values like NaN, Infinity
    sql.createScalarFunction('returnNaN', () => NaN);
    this.functionsTeardown.push('returnNaN');
    
    sql.createScalarFunction('returnInfinity', () => Infinity);
    this.functionsTeardown.push('returnInfinity');

    // Check how special values are handled in SQL
    result = [...sql.exec('SELECT returnNaN() as nan, returnInfinity() as inf')];
  ...
  async testStringEdgeCases() {
    ...
    sql.createScalarFunction('returnUnicode', () => unicodeString);
    ...
    sql.createScalarFunction('returnLongString', () => longString);
  ...
  async testBlobEdgeCases() {
    ...
    sql.createScalarFunction('returnSmallBlob', () => smallBlob.buffer);
    this.functionsTeardown.push('returnSmallBlob');
    
    let result = [...sql.exec('SELECT returnSmallBlob() as result')];
    console.log(`Small blob result type: ${result[0].result instanceof ArrayBuffer}`);
    ...
    sql.createScalarFunction('returnMediumBlob', () => mediumBlob.buffer);
  ...
  async testEmptyValues() {
    ...
    sql.createScalarFunction('returnEmptyString', () => "");
    this.functionsTeardown.push('returnEmptyString');
    
    let result = [...sql.exec('SELECT returnEmptyString() as result')];
    console.log(`Empty string result: "${result[0].result}", length: ${result[0].result.length}`);
    
    // Test with empty blob
    sql.createScalarFunction('returnEmptyBlob', () => new Uint8Array(0).buffer);
    this.functionsTeardown.push('returnEmptyBlob');
    
    result = [...sql.exec('SELECT returnEmptyBlob() as result')];
    console.log(`Empty blob result type: ${result[0].result instanceof ArrayBuffer}, size: ${result[0].result.byteLength} bytes`);
    
    // Test with undefined (should convert to null)
    sql.createScalarFunction('returnUndefined', () => undefined);
